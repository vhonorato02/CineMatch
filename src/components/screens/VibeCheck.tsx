...
interface VibeCheckProps {
    ...
    onSessionStart: (vibe: string) => void;
}
...
const VibeCheck: React.FC<VibeCheckProps> = ({ isPartnerTyping, onTyping, onSessionStart }) => {
    const [customVibe, setCustomVibe] = useState('');

    const handleCustomVibeChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        const newVal = e.target.value;
        setCustomVibe(newVal);
        onTyping(newVal.length > 0);
    };
            ...
            <div className="grid grid-cols-1 gap-3">
                <div className="bg-slate-900/50 p-4 rounded-2xl border border-white/10">
                    <input
                        ...
                        className="w-full bg-transparent p-2 outline-none font-bold text-sm text-white placeholder:text-slate-600"
                    />
                    {customVibe && (
                        <button
                            onClick={() => onSessionStart(customVibe)}
                            className="mt-4 w-full bg-rose-500 py-3 rounded-xl text-[10px] font-black uppercase shadow-lg active:scale-95 transition-all"
                        >
                            Gerar com IA
                        </button>
                    )}
                ...
                {VIBES.map(v => (
                    <button
                        key={v.id} onClick={() => onSessionStart(v.label)}
